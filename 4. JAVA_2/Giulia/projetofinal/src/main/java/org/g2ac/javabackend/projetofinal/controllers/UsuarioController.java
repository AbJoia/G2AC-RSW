...
public class UsuarioController {
	...
	@PostMapping
	public Usuario add(@Valid @RequestBody Usuario corpo) throws MethodArgumentNotValidException {
		return uService.adicionar(corpo);
	}
	
	@PutMapping("/{id}")
	public Usuario update(@PathVariable Integer id, @Valid @RequestBody Usuario novo) throws ObjectNotFoundException, MethodArgumentNotValidException {
		return uService.atualizar(id, novo);
	}
	
	@DeleteMapping("/{id}")
	public Usuario delete(@PathVariable Integer id) throws ObjectNotFoundException {
